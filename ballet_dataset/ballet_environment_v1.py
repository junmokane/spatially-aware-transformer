...
#from pycolab_ballet import ballet_environment_core_ass as ballet_core
...
DANCER_SHAPES = [
    "triangle",
    "empty_square", "plus", "inverse_plus", "ex", "inverse_ex",
...
class BalletEnvironment(dm_env.Environment):
  ...
  def _game_factory(self):
    ...
    dancers = []
    for i in range(self._num_dancers):
      _dancer = []
      _dancer.append(dancer_chars[i])
      _dancer.append(positions[i])
      _dancer.append(self._rng.choice(motions))
      found = False
      while not found:
        _shape = self._rng.choice(shapes)
        _color = self._rng.choice(colors)
        duplicate = False
        for j in range(len(dancers)):
          if _shape == dancers[j][3] and _color == dancers[j][4]:
            duplicate = True
            break
        if not duplicate:
          _dancer.append(_shape)
          _dancer.append(_color)
          found = True
      dancers.append(_dancer)
    rooms = np.random.permutation(self._num_dancers) % 8
    label_time = (rooms == np.random.randint(8)).nonzero()[0]
    '''
    ...
    self.info_dict = {
      'positions': positions,
      'rooms': rooms,
      'motions': [motions.index(dancers[i][2]) for i in range(self._num_dancers)],
